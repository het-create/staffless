...
exportDocxBtn.addEventListener("click", async () => {
  ...
  try {
    const res = await fetch("/api/export/docx", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        product: lastBrief ? lastBrief.product : "STAFFLESS Campaign",
        content: deliverableContentEl.textContent,
      }),
    });
    ...
    a.href = url;
    a.download = `${(lastBrief?.product || "staffless-campaign").replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.docx`;
    ...
    exportDocxBtn.textContent = "Download as Word";
  }
});
